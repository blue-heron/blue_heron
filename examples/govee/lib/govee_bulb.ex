defmodule GoveeBulb do
  @moduledoc """
  Sample ATT application that can control the Govee Light Bulb
  ...
  def handle_info(
    ...
    Logger.info("Trying to connect to GoveeBulb #{inspect(addr, base: :hex)}")
  ...
  def handle_info({BlueHeron.ATT.Client, conn, %ConnectionComplete{}}, %{conn: conn} = state) do
    Logger.info("GoveeBulb connection established")
  ...
  def handle_info({BlueHeron.ATT.Client, _, %DisconnectionComplete{reason_name: reason}}, state) do
    Logger.warning("GoveeBulb connection dropped: #{reason}")
  ...
  def handle_call({:set_color, rgb}, _from, state) do
    ...
    case BlueHeron.ATT.Client.write(state.conn, 0x0015, <<value::binary-19, checksum::8>>) do
      :ok ->
        Logger.info("Setting GoveeBulb Color: ##{inspect(rgb, base: :hex)}")
        {:reply, :ok, state}

      error ->
        Logger.info("Failed to set GoveeBulb color")
        {:reply, error, state}
    end
  end
