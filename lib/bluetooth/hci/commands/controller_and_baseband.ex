defmodule Bluetooth.HCI.Command.ControllerAndBaseband do
  ...
  * OGF: `#{inspect(@ogf, base: :hex)}`
  ...
  def list() do
    ...
    |> Enum.filter(
      &match?(["Bluetooth", "HCI", "Command", "ControllerAndBaseband", _mod], Module.split(&1))
    )
  end
  ...
  defmacro __using__(opts) do
    quote location: :keep, bind_quoted: [opts: opts] do
      ...
      use Bluetooth.HCI.Command, Keyword.put(opts, :ogf, CaB.__ogf__())

      @ocf ocf
      @opcode Bluetooth.HCI.Command.opcode(CaB.__ogf__(), @ocf)
