defmodule Bluetooth.HCI.Command.InformationalParameters do
  ...
  * OGF: `#{inspect(@ogf, base: :hex)}`
  ...
  @spec list :: [module()]
  def list() do
    Application.spec(:bluetooth, :modules)
    |> Enum.filter(
      &match?(["Bluetooth", "HCI", "Command", "InformationalParameters", _mod], Module.split(&1))
    )
  end
  ...
  defmacro __using__(opts) do
    quote location: :keep, bind_quoted: [opts: opts] do
      ...
      use Bluetooth.HCI.Command, Keyword.put(opts, :ogf, IP.__ogf__())

      @ocf ocf
      @opcode Bluetooth.HCI.Command.opcode(IP.__ogf__(), @ocf)
