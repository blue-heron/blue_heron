defmodule BlueHeron.Util.Scan do
    ...
    spawn(fn ->
      ...
      receive do
        {:BLUETOOTH_EVENT_STATE, :HCI_STATE_WORKING} ->
          BlueHeron.hci_command(ctx, %BlueHeron.HCI.Command.LEController.SetScanEnable{
            le_scan_enable: true
          })
      after
        1000 ->
  ...
  defp _scan_print(ctx, devices) do
    receive do
      {:HCI_EVENT_PACKET,
       %BlueHeron.HCI.Event.LEMeta.AdvertisingReport{
         devices: new_devices
       }} ->
        devices =
  ...
  def scan_for_led_bulb(ctx) do
    ...
    receive do
      {:HCI_EVENT_PACKET,
       %BlueHeron.HCI.Event.LEMeta.AdvertisingReport{
         devices: [
           %BlueHeron.HCI.Event.LEMeta.AdvertisingReport.Device{
             address: addr,
             data: ["\tMinger" <> _]
           }
         ]
